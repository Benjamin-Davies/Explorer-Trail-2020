...
import { TEAM_NAME_ONE, TEAM_NAME_TWO } from '../constants/team-names';
...
export class AuthService {
  ...
  async emailLogin(email: string, password: string): Promise<void> {
    return this.firebaseService.emailSignin(email, password).then((response) => this.setUser(response.user));
  }
   ...
   */
  setProfile() {
    const obs = this.api.getProfile(this._user.id).pipe(
      map((res) => {
