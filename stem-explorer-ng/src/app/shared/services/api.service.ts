...
export class ApiService {
  ...
  getExternalContent() {
    return this.http.get<ExternalContent[]>(`/ExternalContent`);
  }

  getFeaturedLocations() {
    return this.http.get<FeaturedLocation[]>(`/Locations/Featured`);
  }
  ...
  validateAnswer(levelUid: number, answer: string) {
    return this.http.post<boolean>(`/ChallengeLevels/${levelUid}/ValidateAnswer`, JSON.stringify(answer), {
      headers: {
  ...
  getCurrentUser() {
    return this.http.get<User>(`/User/CurrentUser`);
  }

  registerUser(profileInfo: Profile) {
    return this.http.post<User>(`/User/RegisterUser`, profileInfo);
  }

  updateUser(userInfo: User) {
    return this.http.put<User>(`/User/CurrentUser`, userInfo);
  }

  getProgress(profileId: number) {
    return this.http.get<Progress[]>(`/Progress/${profileId}`);
  }

  levelCompleted(profileId, levelId: number[], correct: boolean) {
    return this.http.post(`/Progress/LevelCompleted`, {
      levelId,
      correct,
      profileId,
    });
  ...
  getProfile(userId: string) {
    return this.http.get<Profile>(`/Profile?userId=${userId}`);
  }
  ...
  updateProfile(profile: Profile) {
    return this.http.put<Profile>(`/Profile/Update`, profile);
  }
}
